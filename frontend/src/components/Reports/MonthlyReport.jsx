...
export function MonthlyReport({ onBack }) {
  ...
  const fetchReport = async () => {
    try {
      setLoading(true);
      const result = await reportAPI.getMonthlyReport(month, year);
      setReport(result);
    } catch (error) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-900">Monthly Report - {monthName} {year}</h2>
        <button
          onClick={onBack}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <p className="text-2xl font-bold text-blue-600 mt-2">
              {formatCurrency((report.sales_summary?.total_sales || 0) - (report.expense_summary?.total_expenses || 0))}
            </p>
          </div>
